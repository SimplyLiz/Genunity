...
Usage:
    python -m biolab.pipeline.evidence_runner <source>
    python -m biolab.pipeline.evidence_runner --list

Each evidence source module exposes:
    run_<source>(db, genes, http) -> int   (count of evidence rows stored)

Sources are discovered at runtime via the plugin registry (biolab.registry).
Contrib modules like biolab.contrib.dnasyn register themselves on import.
"""
...
def _ensure_contrib_loaded() -> None:
    ...
    try:
        import biolab.contrib.dnasyn  # noqa: F401
    except Exception:
...
def list_sources() -> list[dict[str, str]]:
    ...
    from biolab.registry import check_source_availability, list_registered_sources
...
def run_source(source: str, unknown_only: bool = True) -> int:
    ...
    """
    from biolab.registry import load_runner, list_registered_sources
...
def run_all_sources(unknown_only: bool = True) -> dict[str, int]:
    ...
    from biolab.registry import list_registered_sources

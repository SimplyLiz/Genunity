...
from cellforge.api.schemas import UserInfoResponse
from cellforge.db.models import TokenTransaction, User
...
async def get_me(
    ...
    """Return current user info and token balance."""
...
async def clerk_webhook(request: Request, db: AsyncSession = Depends(get_db)) -> dict[str, str]:
    """Handle Clerk user.created/user.updated webhooks."""
